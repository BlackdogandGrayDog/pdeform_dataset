...
def geotrf(Trf, pts, ncol=None, norm=False):
    ...
    if (isinstance(Trf, torch.Tensor) and isinstance(pts, torch.Tensor) and
        ...
        if Trf.shape[-1] == d:
            ...
            raise ValueError(f'bad shape, not ending with 3 or 4, for pts.shape={pts.shape}')
    else:
        if Trf.ndim >= 3:
            n = Trf.ndim - 2
...
def normalize_pointcloud(pts1, pts2, norm_mode='avg_dis', valid1=None, valid2=None, ret_factor=False):
    ...
    if norm_mode == 'avg':
        ...
        if dis_mode == 'dis':
            ...
            raise ValueError(f'bad dis_mode={dis_mode}')
        ...
        if norm_mode == 'avg':
            ...
            raise ValueError(f'bad norm_mode={norm_mode}')
