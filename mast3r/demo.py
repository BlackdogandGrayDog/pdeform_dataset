...
def get_3D_model_from_scene(silent, scene_state, min_conf_thr=2, as_pointcloud=False, mask_sky=False,
    ...
    save_path = "./test_img/simulator_nr_1/mast3r_pred_rot_trans.txt"  # <- CHANGE THIS to your desired output path
    ...
    with open(save_path, "w") as f:
        for i, cam in enumerate(cams2world):
            ...
            f.write("Rotation (3x3, camera-to-world):\n")
            for row in rot:
                f.write(" ".join(f"{val:.8f}" for val in row) + "\n")
            f.write("Translation (3x1, camera-to-world):\n")
            f.write(" ".join(f"{val:.8f}" for val in trans) + "\n\n")
