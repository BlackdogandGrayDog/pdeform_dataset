...
def create_grid_points(H, W, grid_size, margin_ratio=0.1, use_random_margin=False, use_random_offset=False, use_jitter=False, jitter_std=1.5):
...
def fast_reciprocal_NNs_SP(pts1, pts2, x1, y1, pixel_tol=0, ret_basin=False,
                            device='cuda', ret_xy=True, ret_index=False, **matcher_kw):
    H1, W1, DIM1 = pts1.shape
    H2, W2, DIM2 = pts2.shape
    assert DIM1 == DIM2
    ...
    query_flat = np.int32(x1 + W1 * y1)  # preserve original order
    unique_flat = np.unique(query_flat)
    xy1 = unique_flat
    xy2 = np.full_like(xy1, -1)
    old_xy1 = xy1.copy()
    old_xy2 = xy2.copy()
    ...
    matched_xy1, matched_xy2 = merge_corres(xy1[~notyet], xy2[~notyet], (H1, W1), (H2, W2),
                                            ret_xy=ret_xy, ret_index=ret_index)

    # === New additions: full_xy1, full_xy2, status ===
    full_xy1 = np.stack([x1, y1], axis=1).astype(np.float32)
    full_xy2 = np.full_like(full_xy1, -1, dtype=np.float32)
    status = np.zeros(len(full_xy1), dtype=np.uint8)

    # Re-map flat indices to original positions
    matched_set = set(xy1[~notyet])
    flat_to_xy = {}
    if ret_xy:
        for a, b in zip(matched_xy1, matched_xy2):
            flat_idx = int(a[0]) + W1 * int(a[1])
            flat_to_xy[flat_idx] = b

    for i, flat in enumerate(query_flat):
        if flat in matched_set and flat in flat_to_xy:
            full_xy2[i] = flat_to_xy[flat]
            status[i] = 1

    # Return everything
    if ret_basin:
        return full_xy1, full_xy2, status, basin
    return full_xy1, full_xy2, status, basin
